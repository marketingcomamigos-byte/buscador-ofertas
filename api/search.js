...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const [source, mercadoLivreProducts] = await Promise.all([
      brands.length ? fetchProductPage(apiKey, q, page, limit, organizationIds) : Promise.resolve({items:[],total:0}),
      page===1 ? fetchMercadoLivreImports(q, 100).catch(()=>[]) : Promise.resolve([]),
      Promise.resolve([])
    ]);
    ...
    const importedProducts = [...mercadoLivreProducts];
